...
print('Accumulated lines of code for Create_CF_report', count_lines_in_directory(directory))
clear_screen(3)
...
if returning_client_data:
    # Document_ID = '1LmLja6DTVrwvOcxcrBdfabPxgVjORzQEvGZ24l2FGkY' # Your Professionals YS-one gateway to test form
    def get_document_id(CF_report_number):
        document_dict = {
            'XJ': 'provide new link', # Expro
            'YS': '1LmLja6DTVrwvOcxcrBdfabPxgVjORzQEvGZ24l2FGkY', 
            'DD': 'provide new link', # Deep Dive
            'IP': 'provide new link'  # Individual Professional
        }
        client_key = CF_report_number[:2] # get the first two characters

        # check if client_key exists in the dictionary
        if client_key in document_dict:
            return document_dict[client_key]
        else:
            raise ValueError('Invalid CF_report_number. Could not find a matching Document_ID.')
    
    Document_ID = get_document_id(CF_report_number)
...
if returning_client_data:
    ...
    # User inputs the client who issued the test
    def get_client_name(CF_report_number):
        client_dict = {
            'XJ': 'Expro Engineering B.V.',
            'YS': 'Your Professionals B.V.',
            'DD': 'Deep Dive',
            'IP': ''
        }
        ...
        # check if client_key exists in the dictionary
        if client_key in client_dict:
            return client_dict[client_key]
        else:
            return input('Geef de naam van de opdrachtgever in: ')
    ...
    try:
        ...
        relation = None
        meaning = None
        print('No Data could be retreived, please refresh data or enter data manually.')
...
print('from Create_CF_report, language set to : ', get_report_language()) # To test availability of the global variable
clear_screen(2)
...
if returning_client_data:
        ...
        test_subject_name =decrypted_CF_data['name_client']   # name candidate eg 'Ivar Koehorst'
        ...
        print('Consistency of test ',CF_report_number,' is ', str(consistency))
        clear_screen(1.5)
        ...
        for i in range(51, 55):  # loop from 51 to 54
            ...
            print(decrypted_CF_data[answer_key])
        ...
        print ('Total instances of fear: ', open_answer_fear)
        print ('Total instances of courage: ', open_answer_courage)
        ...
        # print(open_answers_correlate)
        # print(free_energy)
        open_answer_consistency = abs(open_answers_correlate - free_energy) < 25
        print('open answer consistency with courage and fear score is: ', open_answer_consistency)
        clear_screen(3)
        ...
        # print(courage)
        total_effectivity = sum(courage) - sum(fear)
        # print(total_effectivity)
        # print(decrypted_CF_data['free_energy'])
        ...
        def test_outcome(value=float):
            ...
            # # Use the function
            # print(test_outcome(6))  # Returns: 'Growth'
            # print(test_outcome(-6))  # Returns: 'Fixed'
            # print(test_outcome(0))  # Returns: 'Doubt'
...
def create_CF_report(terminalmessage:bool = True):
    ...
    def count_gender_words(input_txt:str):
        ...
        print(f"Occurrences of 'zichzelf': {count_zichzelf}")
        print(f"Occurrences of 'hij ': {count_hij}")
   ...
   # Define and place footnote
    ...
    def create_title_page():
        ...
        # Kandidaat naam
        y -= 2*line_height
        center(test_subject_name,font_default,18, y)
    ...
    def create_page_courage_and_fear():
        ...
        if female_gender: 
            print('Changing gender to female in text..')
    ...
    print('Thanks for using create_CF_report!')
    clear_screen(2)
